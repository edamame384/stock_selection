...
def run_dataset(
    ...
    for date in dates:
        ...
        rule_name = "downtrend" if phase_name == "crash" else mapping.get(phase_name, "condition2")
...
from projects.shikiho_text_parser.compare_q2_2024_sector_filtered_chart_tuning_candidate import select_variant as _sector_select_variant

SECTOR_VARIANT_NAME = "top4_chart_v2_balanced"
SECTOR_POST_CRASH_TRADING_DAYS = 20
FIXED_ALLOC_CAP = 500_000.0


def in_post_crash_window(date: pd.Timestamp, phase_map: pd.Series, phase_shift_days: int, trading_days: int) -> bool:
    hist = lagged_phase_history(date, phase_map, phase_shift_days)
    if hist.empty:
        return False
    recent = hist.tail(trading_days)
    return any(str(x) == "crash" for x in recent.tolist())


def fixed_alloc_shares(fill_price: float, cash: float) -> int:
    lot_cost = fill_price * 100.0
    if lot_cost <= 0:
        return 0
    if lot_cost < FIXED_ALLOC_CAP:
        max_lots_by_cap = int(FIXED_ALLOC_CAP // lot_cost)
        max_lots_by_cash = int(cash // lot_cost)
        lots = min(max_lots_by_cap, max_lots_by_cash)
        return lots * 100 if lots >= 1 else 0
    return 100 if cash >= lot_cost else 0


def run_q2_postcrash_sector_alloc_candidate() -> dict:
    spec = TableSpec(
        "q2_2024",
        ROOT / "projects" / "quarterly_ranker" / "output" / "q2_2024_pre_analysis_20240630_aligned" / "q2_2024_pre_shikiho_feature_ranking.csv",
        ROOT / "projects" / "quarterly_ranker" / "output" / "q2_2024_pre_analysis_20240630_aligned" / "operational" / "q2_2024_pre_selected_candidates_operational.csv",
    ...
    trade_dates = sorted(set().union(*[set(df[(df.index >= start_date) & (df.index <= end_date)].index.tolist()) for df in price_map_all.values()]))
    if not trade_dates:
        return {"dataset": spec.name, "final_capital": spec.initial_capital, "total_return_pct": 0.0, "num_buys": 0, "max_drawdown_pct": 0.0}

    standard_tables = build_interval_standard_tables(spec.name, detail_df, price_map_all, trade_dates, 10)
    crash_tables = build_interval_crash_tables(detail_df, price_map_all, trade_dates, 10)
    anchors, anchor_for_date = build_rebalance_schedule(trade_dates, 10)

    sector_tables = {}
    for anchor_key in anchors:
        prior_trade_dates = [d for d in trade_dates if d < anchor_key]
        basis_date = prior_trade_dates[-1] if prior_trade_dates else trade_dates[0] - pd.Timedelta(days=1)
        snapshot = build_snapshot(detail_df, price_map_all, basis_date)
        sector_tables[anchor_key] = _sector_select_variant(snapshot.copy(), SECTOR_VARIANT_NAME).drop_duplicates(subset=["ticker"], keep="first").reset_index(drop=True)

    union_tickers = set()
    for tbl_map in (standard_tables, crash_tables, sector_tables):
        for tbl in tbl_map.values():
            if not tbl.empty:
                union_tickers.update(tbl["ticker"].astype(str).tolist())
    price_map = {ticker: df for ticker, df in price_map_all.items() if ticker in union_tickers}
    dates = sorted(set().union(*[set(df[(df.index >= start_date) & (df.index <= end_date)].index.tolist()) for df in price_map.values()]))
    ...
    earnings_cache = fetch_earnings_cache(tickers)
    irbank_cache = fetch_irbank_earnings_cache(tickers)
    local_cache = load_local_disclosure_dates(tickers)
    merged_frames = [df for df in [local_cache[["ticker", "earnings_date"]] if not local_cache.empty else pd.DataFrame(), irbank_cache[["ticker", "earnings_date"]] if not irbank_cache.empty else pd.DataFrame(), earnings_cache[["ticker", "earnings_date"]] if not earnings_cache.empty else pd.DataFrame()] if not df.empty]
    merged = pd.concat(merged_frames, ignore_index=True) if merged_frames else pd.DataFrame(columns=["ticker", "earnings_date"])
    earnings_map = build_earnings_map(merged)
    ...
    for date in dates:
        phase_name = projected_phase_name(date, phase_map, 1, "difficult_v11")
        anchor_key = anchor_for_date[date]
        standard_lookup = standard_tables[anchor_key].set_index("ticker").to_dict("index") if anchor_key in standard_tables and not standard_tables[anchor_key].empty else {}
        crash_lookup = crash_tables[anchor_key].set_index("ticker").to_dict("index") if anchor_key in crash_tables and not crash_tables[anchor_key].empty else {}
        sector_lookup = sector_tables[anchor_key].set_index("ticker").to_dict("index") if anchor_key in sector_tables and not sector_tables[anchor_key].empty else {}

        post_crash_window = in_post_crash_window(date, phase_map, 1, SECTOR_POST_CRASH_TRADING_DAYS)
        use_sector_trade = post_crash_window and phase_name != "crash"

        if phase_name == "crash":
            active_lookup = {}
            rule_name = "no_trade"
        elif use_sector_trade:
            active_lookup = sector_lookup
            rule_name = "q2_defensive"
        else:
            active_lookup = standard_lookup
            rule_name = mapping.get(phase_name, "condition2")

        signal_today = {}
        score_today = {}
        basis_date_today = {}
        earnings_block_today = {}
        earnings_post_block_today = {}
        ...
        for ticker in price_map:
            ...
            if ticker not in active_lookup or rule_name == "no_trade":
                signal_today[ticker] = False
                continue
            metrics = metrics_cache.get(ticker, {}).get(date, {})
            if not metrics:
        ...
        for ticker in list(positions.keys()):
            ...
            if exit_reason is not None:
                shares = positions[ticker]["shares"]
                cash += shares * float(exit_price)
                trade_rows.append({"date": date.strftime("%Y-%m-%d"), "ticker": ticker, "action": "SELL", "price": float(exit_price), "shares": int(shares), "reason": exit_reason, "nikkei_phase": phase_name, "post_crash_sector_window": use_sector_trade})
                del positions[ticker]
        ...
        for ticker, _score in candidates:
            ...
            if day_high < trigger_price:
                continue
            fill_price = max(day_open, trigger_price)
            shares = fixed_alloc_shares(fill_price, cash)
            if shares >= 100 and shares * fill_price <= cash:
                cash -= shares * fill_price
                positions[ticker] = {"shares": shares, "entry_price": fill_price, "rule_name": rule_name}
                trade_rows.append({"date": date.strftime("%Y-%m-%d"), "ticker": ticker, "action": "BUY", "price": float(fill_price), "shares": int(shares), "reason": f"{rule_name}_entry", "nikkei_phase": phase_name, "post_crash_sector_window": use_sector_trade})
                buy_count += 1
    ...
    for ticker in list(positions.keys()):
        ...
        if len(usable_idx):
            px = float(df.loc[usable_idx[-1], "Close"])
            cash += positions[ticker]["shares"] * px
            trade_rows.append({"date": latest_date.strftime("%Y-%m-%d"), "ticker": ticker, "action": "SELL", "price": px, "shares": int(positions[ticker]["shares"]), "reason": "end_of_backtest", "nikkei_phase": projected_phase_name(latest_date, phase_map, 1, "difficult_v11"), "post_crash_sector_window": in_post_crash_window(latest_date, phase_map, 1, SECTOR_POST_CRASH_TRADING_DAYS)})
        del positions[ticker]
    ...
    return {
        ...
        "trade_log": pd.DataFrame(trade_rows),
        "equity_curve": equity_df,
        "sector_variant": SECTOR_VARIANT_NAME,
        "post_crash_trading_days": SECTOR_POST_CRASH_TRADING_DAYS,
        "alloc_cap": FIXED_ALLOC_CAP,
    }


def main() -> int:
    parser = argparse.ArgumentParser(description="q2_2024 candidate: crash no-trade, post-crash 20 trading days sector trade, fixed allocation cap.")
    parser.add_argument("--output-dir", type=Path, default=ROOT / "projects" / "shikiho_text_parser" / "output" / "phase_adaptive_practical_v35_postcrash_sector_alloc_q2_candidate")
    args = parser.parse_args()
    args.output_dir.mkdir(parents=True, exist_ok=True)
