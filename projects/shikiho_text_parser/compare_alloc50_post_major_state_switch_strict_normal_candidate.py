...
OUT_DIR = ROOT / "projects" / "shikiho_text_parser" / "output" / "compare_alloc50_post_major_state_switch_strict_normal_candidate"
...
NORMAL_RET5_MIN = 0.03
NORMAL_CONTINUITY_DAYS = 5
NORMAL_CONTINUITY_MIN_POSITIVE = 4
...
def classify_sector_state(table_df: pd.DataFrame) -> dict:
    if table_df is None or table_df.empty or "sector" not in table_df.columns:
        return {"mode": "concentrated", "top_share": 1.0, "hhi": 1.0, "num_sectors": 0}
...
def post_major_stock_allowed(
    date: pd.Timestamp,
    phase_name: str,
    post_major: bool,
    post_major_phase: str,
    phase_state: pd.DataFrame,
) -> tuple[bool, str]:
    if not post_major:
        return False, "not_post_major"
    if phase_name == "downtrend":
        return False, "current_downtrend"
    if post_major_phase in {"capitulation_end", "settling"}:
        return True, post_major_phase
    if post_major_phase != "normal":
        return False, f"phase_{post_major_phase}"
    hist = phase_state.loc[phase_state.index < date].tail(NORMAL_CONTINUITY_DAYS)
    if len(hist) < NORMAL_CONTINUITY_DAYS:
        return False, "normal_short_history"
    positive_count = int(hist["phase_name"].astype(str).isin({"normal", "uptrend", "stable"}).sum())
    if positive_count < NORMAL_CONTINUITY_MIN_POSITIVE:
        return False, f"normal_continuity_{positive_count}"
    last_ret5 = pd.to_numeric(hist.iloc[-1]["ret5"], errors="coerce")
    if pd.isna(last_ret5) or float(last_ret5) < NORMAL_RET5_MIN:
        return False, f"normal_ret5_{0.0 if pd.isna(last_ret5) else float(last_ret5):.4f}"
    return True, "normal_strong"
...
def run_dataset(spec: TableSpec, phase_df: pd.DataFrame, earnings_map: dict[str, list[pd.Timestamp]]) -> dict:
    ...
    for date in dates:
        ...
        stock_post_major_active, stock_gate_reason = post_major_stock_allowed(
            date=date,
        ...
        state_rows.append({
            ...
            "stock_post_major_active": bool(stock_post_major_active),
            "stock_gate_reason": stock_gate_reason,
        ...
        for ticker in list(positions.keys()):
            ...
            if positions[ticker].get("entry_mode") == "post_major_stock":
                if exit_reason is None and ret >= STOCK_TP:
                    ...
                    if exit_reason is None and (not (stock_post_major_active and sector_state["mode"] == "dispersed") or not stock_sig):
                        exit_reason = "sell_signal"
                        exit_price = close_price
            else:
                ...
                if len(basis_idx):
                    ...
                    if etf_ok:
                        ...
                        if shares >= 100 and shares * fill_price <= cash:
                            ...
                            buy_count += 1
        elif stock_post_major_active and sector_state["mode"] == "dispersed":
            stock_candidates = []
            for ticker in [t for t in price_map if t != ETF_TICKER]:
...
def main() -> int:
    ...
    for name, spec in specs.items():
        ...
        out_dir.mkdir(parents=True, exist_ok=True)
        payload = {
            "variant": "alloc50_post_major_state_switch_strict_normal_2516_or_stock_prev_high_break_top2_tp6_sl3",
            "dataset": name,
            "concentrated_mode": f"ETF {ETF_TICKER} prev_high_break tp6/sl3",
            "dispersed_mode": f"stock prev_high_break top{STOCK_TOP_N} tp6/sl3 with no current downtrend and strong 1w normal filter",
