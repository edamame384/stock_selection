...
def run_dataset_with_postcrash_highvol_normal_filter(
    spec: mod.TableSpec,
    phase_map: pd.Series,
...
def save_run(output_dir: Path, name: str, result: dict) -> dict:
    ...
    payload["variant"] = "normal_after_postcrash_highvol_candidate"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run v3.5 candidate with normal_after_postcrash_highvol treated as no_trade.")
...
def main() -> int:
    ...
    for spec in specs:
        result = run_dataset_with_postcrash_highvol_normal_filter(
            spec,
            phase_map,
            earnings_map,
