...
OUT_DIR = ROOT / "projects" / "shikiho_text_parser" / "output" / "compare_2025_2q_post_major_crash_stock_trend_candidate"
...
def run_backtest() -> dict:
    ...
    for date in dates:
            ...
            if positions[ticker].get("entry_mode") == "post_major_trend":
                if exit_reason is None and ret >= POST_MAJOR_TP:
                    ...
                    if len(basis_idx) and post_major_active:
                        hold_sig, _ = prev_high_break_signal(df, basis_idx[-1], date)
        ...
        for ticker in price_map:
            ...
            if post_major_active:
                ...
                if not sig or trigger is None:
                    continue
                candidates.append((ticker, score_today.get(ticker, 0.0), float(trigger)))
            else:
        ...
        for ticker, _score, trigger_price in candidates:
            df = price_map[ticker]
            day = df.loc[date]
            ...
            positions[ticker] = {
                ...
                "entry_mode": "post_major_trend" if post_major_active else "base",
            }
            reason = "post_major_prev_high_break_entry" if post_major_active else f"{rule_name}_entry"
    ...
    payload = {
        "variant": "2025_2q_low_concentration_post_major_stock_prev_high_break_tp6_sl3",
        "dataset": "2025_2q",
...
def main() -> int:
    ...
    print(json.dumps({k: v for k, v in result.items() if k not in {"trade_log", "equity_curve"}}, ensure_ascii=False, indent=2))
    return 0
