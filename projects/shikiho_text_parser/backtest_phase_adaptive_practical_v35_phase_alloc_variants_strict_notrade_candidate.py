...
ALLOC_VARIANTS = {
    ...
    "weak_uptrend_only_50": {
        "high_vol": None,
        "weak_uptrend": 500_000.0,
        "settling": None,
        "reversal_up": None,
        "normal": None,
        "stable": None,
...
def alloc_shares_variant(fill_price: float, cash: float, phase_name: str, is_weak_uptrend: bool, variant: str) -> int:
    lot_cost = fill_price * 100.0
    if lot_cost <= 0:
        return 0
    cfg = ALLOC_VARIANTS[variant]
    cap = cfg.get('weak_uptrend') if is_weak_uptrend else cfg.get(phase_name)
    if cap is None:
        return int(cash // lot_cost) * 100
    max_lots_by_cap = int(cap // lot_cost)
...
def run_dataset(
    ...
    alloc_variant: str = "highvol_only_50",
) -> dict:
    ...
    for date in dates:
        ...
        active_lookup = {} if rule_name == "no_trade" else standard_lookup
        ...
        for ticker, _score in candidates:
            ...
            shares = alloc_shares_variant(fill_price, cash, phase_name, is_weak_uptrend, alloc_variant)
...
def main() -> int:
    ...
    parser.add_argument("--alloc-variant", type=str, choices=sorted(ALLOC_VARIANTS.keys()), default="highvol_only_50")
    parser.add_argument(
        "--phase-proxy-mode",
        type=str,
        default="lagged",
    ...
    for spec in specs:
        ...
        payload = {
            ...
            "alloc_variant": args.alloc_variant,
