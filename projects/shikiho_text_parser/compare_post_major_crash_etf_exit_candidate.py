...
OUT_DIR = ROOT / "projects" / "shikiho_text_parser" / "output" / "post_major_crash_etf_exit_candidate"
...
EXIT_VARIANTS = [
    ...
    {"name": "tp4_sl3", "take_profit_pct": 4.0, "stop_loss_pct": 3.0, "max_hold_days": None},
...
def regime_allows_entry(date: pd.Timestamp, phase_df: pd.DataFrame) -> bool:
    ...
    if hist.empty:
        return False
    recent = hist.tail(10)
    current_phase = str(recent.iloc[-1]["phase_name"])
    if current_phase not in {"high_vol", "capitulation_end", "settling"}:
        return False
    return bool(recent["major_crash"].any())
...
def main() -> int:
    ...
    best = summary_df.groupby("dataset").head(5).reset_index(drop=True)
