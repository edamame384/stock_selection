...
def run_dataset(spec: TableSpec, phase_map: pd.Series, earnings_map: dict[str, list[pd.Timestamp]], earnings_post_days: int) -> dict:
    ...
    for date in dates:
        ...
        earnings_entry_window_today: dict[str, bool] = {}

        for ticker in price_map:
            df = price_map[ticker]
            dse = trading_days_since_prev_earnings(date, df, earnings_map.get(ticker, []))
            earnings_entry_window_today[ticker] = dse is not None and 1 <= dse <= earnings_post_days
            ...
            if not earnings_entry_window_today[ticker]:
                sig = False
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run practical phase-adaptive v3.1 with configurable post-earnings business-day entry window.")
    ...
    rows = []
    for spec in specs:
        result = run_dataset(spec, phase_map, earnings_map, args.earnings_post_days)
        out_dir = args.output_dir / spec.name if args.batch else args.output_dir
        out_dir.mkdir(parents=True, exist_ok=True)
        payload = {
            "mapping_name": f"practical_phase_adaptive_v31_post_earnings{args.earnings_post_days}",
            "mapping_ja": f"実務向け局面切替v3.1_決算後{args.earnings_post_days}営業日",
