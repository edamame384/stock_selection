...
def classify_monthly_standard_mode(month_key: pd.Timestamp, phase_map: pd.Series) -> str:
    hist = phase_map.loc[phase_map.index < month_key]
    if hist.empty:
        return "standard"
    recent = hist.tail(10)
    latest = str(recent.iloc[-1])
    if latest in {"high_vol", "settling", "reversal_up", "capitulation_end"}:
        return "intermediate"
    if any(str(x) in {"crash", "surge"} for x in recent.tolist()):
        return "intermediate"
    return "standard"
...
def build_monthly_standard_tables(
    ...
    for month_key in month_starts:
        ...
        mode = classify_monthly_standard_mode(month_key, phase_map)
        if mode == "intermediate":
            tables[month_key] = build_post_crash_broad_table(snapshot)
        else:
...
def run_dataset(
    ...
    standard_tables = build_monthly_standard_tables(spec.name, detail_df, price_map_all, trade_dates, phase_map)
...
def collect_relevant_tickers(spec: TableSpec) -> list[str]:
    ...
    if not trade_dates:
        return []
    standard_tables = build_monthly_standard_tables(spec.name, detail_df, price_map_all, trade_dates, phase_map=load_phase_map(PHASE_CSV))
    crash_tables = build_monthly_crash_tables(detail_df, price_map_all, trade_dates)
    union_tickers: set[str] = set()
    for tbl in standard_tables.values():
        if not tbl.empty:
