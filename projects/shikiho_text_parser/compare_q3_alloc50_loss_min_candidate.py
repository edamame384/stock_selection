...
OUT_DIR = ROOT / "projects" / "shikiho_text_parser" / "output" / "q3_alloc50_loss_min_candidate"
...
def run_variant(spec: mod.TableSpec, phase_map: pd.Series, earnings_map: dict[str, list[pd.Timestamp]], name: str) -> dict:
    ...
    def custom_mapping() -> dict[str, str]:
        ...
        if name in {"cap_end_no_trade", "combo_highvol10_no_trade"}:
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Compare q3 alloc50 loss-min candidates.")
