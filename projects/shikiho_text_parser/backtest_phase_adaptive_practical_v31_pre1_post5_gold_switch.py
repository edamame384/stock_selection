...
GOLD_ENTRY_LIMIT_PCT = 1.0
GOLD_TAKE_PROFIT_PCT = 5.0
GOLD_STOP_LOSS_PCT = 4.0
...
def gold_defensive_signal(df: pd.DataFrame, basis_date: pd.Timestamp) -> tuple[bool, float]:
    hist = df.loc[df.index <= basis_date].copy()
    if hist.empty or len(hist) < 5:
        return False, 0.0
    close = float(hist.iloc[-1]["Close"])
    ma5 = float(hist["Close"].tail(5).mean())
    ma20 = float(hist["Close"].tail(min(20, len(hist))).mean())
    signal = close > ma5 and ma5 >= ma20
    score = max(close / ma5 - 1.0, 0.0) + 0.5 * max(close / ma20 - 1.0, 0.0)
    return signal, float(score)


def gold_rebound_open_signal(df: pd.DataFrame, basis_date: pd.Timestamp) -> tuple[bool, float]:
    hist = df.loc[df.index <= basis_date].copy()
    if hist.empty or len(hist) < 20:
        return False, 0.0
    hist["ma5"] = hist["Close"].rolling(5).mean()
    hist["hh20"] = hist["Close"].rolling(20).max()
    hist["ret5"] = hist["Close"].pct_change(5)
    row = hist.iloc[-1]
    if pd.isna(row["ma5"]) or pd.isna(row["hh20"]) or pd.isna(row["ret5"]):
        return False, 0.0
    dd20 = float(row["Close"] / row["hh20"] - 1.0)
    ret5 = float(row["ret5"])
    signal = dd20 <= -0.05 and ret5 <= -0.02 and float(row["Close"]) > float(row["ma5"])
    score = max(-dd20 - 0.05, 0.0) + max(-ret5 - 0.02, 0.0) + max(float(row["Close"]) / float(row["ma5"]) - 1.0, 0.0)
    return signal, float(score)


ETF_SIGNAL_FUNCS = {
    "breakout": gold_defensive_signal,
    "rebound_open": gold_rebound_open_signal,
    "disabled": lambda df, basis_date: (False, 0.0),
}


def gold_phase_active(
    date: pd.Timestamp,
    phase_map: pd.Series,
    phase_name: str,
    gold_phases: set[str],
    gold_require_recent_phase: str,
    gold_require_recent_phase_days: int,
) -> bool:
    if phase_name not in gold_phases:
        return False
    if gold_require_recent_phase_days <= 0 or not gold_require_recent_phase:
        return True
    prior = phase_map.loc[phase_map.index < date].tail(gold_require_recent_phase_days)
    if prior.empty:
        return False
    return bool((prior.astype(str) == gold_require_recent_phase).any())


def effective_phase_for_etf(
    date: pd.Timestamp,
    phase_map: pd.Series,
    ...
    if phase_shift_days <= 0:
        ...
        phase = phase_map.loc[phase_map.index < date].tail(phase_shift_days)
        phase = phase.tail(1)
    if phase.empty:
        return "normal"
...
def run_dataset(
    ...
    gold_ticker: str,
    gold_phases: set[str],
    gold_allocation_ratio: float,
    gold_signal_name: str,
    gold_require_recent_phase: str,
    gold_require_recent_phase_days: int,
    phase_shift_days: int,
    gold_phase_shift_days: int,
) -> dict:
    ...
    price_map_all = load_price_map(detail_df["ticker"].astype(str).tolist() + [gold_ticker], end_date)
    ...
    gold_signal_func = ETF_SIGNAL_FUNCS[gold_signal_name]
    ...
    for date in dates:
        phase_name = effective_phase_for_etf(date, phase_map, phase_shift_days)
        gold_phase_name = effective_phase_for_etf(date, phase_map, gold_phase_shift_days)
        gold_active_today = gold_phase_active(
            date,
            phase_map,
            gold_phase_name,
            gold_phases,
            gold_require_recent_phase,
            gold_require_recent_phase_days,
        )
        ...
        for ticker in price_map:
            ...
            if ticker == gold_ticker:
                ...
                if not gold_active_today or len(prev_idx) == 0:
                    signal_today[ticker] = False
                    continue
                basis_date = prev_idx[-1]
                sig, signal_score = gold_signal_func(df, basis_date)
                signal_today[ticker] = sig
                score_today[ticker] = signal_score
                basis_date_today[ticker] = basis_date
                continue
            ...
            if gold_active_today and gold_allocation_ratio >= 0.999:
                signal_today[ticker] = False
                continue
        ...
        for ticker in list(positions.keys()):
            ...
            if rule_for_pos == "q2_defensive":
                ...
                sl = TRADING_RULE["stop_loss_pct"] / 100.0
            elif rule_for_pos == "gold_defensive":
                tp = GOLD_TAKE_PROFIT_PCT / 100.0
                sl = GOLD_STOP_LOSS_PCT / 100.0
            else:
                tp = 0.08
                sl = 0.05
            ...
            exit_reason = None
            exit_price = None
            if rule_for_pos != "gold_defensive" and earnings_block_today.get(ticker, False):
                exit_reason = "pre_earnings_exit"
                exit_price = open_price
            elif rule_for_pos != "gold_defensive" and phase_name == "crash":
                prev_idx = df.index[df.index < date]
                if len(prev_idx):
        ...
        if gold_active_today and gold_ticker not in positions and signal_today.get(gold_ticker, False):
            df = price_map[gold_ticker]
            day = df.loc[date]
            ...
            if gold_signal_name == "rebound_open":
                tradable = True
                fill_price = day_open
            else:
                prev_close = float(df.loc[basis_date_today[gold_ticker], "Close"])
                trigger_price = prev_close * (1.0 + GOLD_ENTRY_LIMIT_PCT / 100.0)
                tradable = day_high >= trigger_price
                fill_price = max(day_open, trigger_price) if tradable else None
            if tradable and fill_price is not None:
                alloc = cash * gold_allocation_ratio
                lot_cost = fill_price * 100.0
                shares = int(alloc // lot_cost) * 100
                if shares >= 100 and shares * fill_price <= cash:
                    cash -= shares * fill_price
                    positions[gold_ticker] = {"shares": shares, "entry_price": fill_price, "rule_name": "gold_defensive"}
                    trade_rows.append({
                        "date": date.strftime("%Y-%m-%d"),
                        "ticker": gold_ticker,
                        "action": "BUY",
                        ...
                        "shares": int(shares),
                        "reason": f"gold_{gold_signal_name}_entry",
                        "nikkei_phase": phase_name,
                    })
                    buy_count += 1
        ...
        for ticker, _score in candidates:
            ...
            if ticker == gold_ticker:
                entry_limit = GOLD_ENTRY_LIMIT_PCT
                entry_rule_name = "gold_defensive"
            else:
            ...
            if shares >= 100 and shares * fill_price <= cash:
                cash -= shares * fill_price
                positions[ticker] = {"shares": shares, "entry_price": fill_price, "rule_name": entry_rule_name}
                trade_rows.append({
                    "date": date.strftime("%Y-%m-%d"),
                    "ticker": ticker,
                    "action": "BUY",
                    "price": float(fill_price),
                    "shares": int(shares),
                    "reason": f"{entry_rule_name}_entry",
                    "nikkei_phase": phase_name,
                })
                buy_count += 1
            remaining -= 1
    ...
    for ticker in list(positions.keys()):
        ...
        if len(usable_idx):
            ...
            trade_rows.append({
                ...
                "nikkei_phase": str(phase_map.loc[phase_map.index <= latest_date].tail(1).iloc[0]) if not phase_map.loc[phase_map.index <= latest_date].tail(1).empty else "normal",
            })
        del positions[ticker]
    ...
    return {
        ...
        "gold_phases": sorted(gold_phases),
        "gold_allocation_ratio": gold_allocation_ratio,
        "gold_signal_name": gold_signal_name,
        "gold_require_recent_phase": gold_require_recent_phase,
        "gold_require_recent_phase_days": gold_require_recent_phase_days,
        "phase_shift_days": phase_shift_days,
        "gold_phase_shift_days": gold_phase_shift_days,
        "phase_pnl": phase_pnl,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run practical phase-adaptive v3.1 with pre/post earnings rules and gold ETF switching.")
    ...
    parser.add_argument("--gold-signal-name", type=str, default="breakout", choices=sorted(ETF_SIGNAL_FUNCS.keys()))
    parser.add_argument("--gold-require-recent-phase", type=str, default="")
    parser.add_argument("--gold-require-recent-phase-days", type=int, default=0)
    parser.add_argument("--phase-shift-days", type=int, default=0)
    parser.add_argument("--gold-phase-shift-days", type=int, default=0)
    ...
    irbank_df = fetch_irbank_earnings_cache(all_tickers) if len(all_tickers) else pd.DataFrame(columns=["ticker", "earnings_date", "source"])
    ...
    remote_df = fetch_earnings_cache([t for t in all_tickers if t not in have_local and t not in have_irbank]) if len(all_tickers) else pd.DataFrame(columns=["ticker", "earnings_date"])
    merged = pd.concat([local_df, irbank_df[["ticker", "earnings_date"]] if not irbank_df.empty else pd.DataFrame(columns=["ticker", "earnings_date"]), remote_df], ignore_index=True) if not local_df.empty or not irbank_df.empty or not remote_df.empty else pd.DataFrame(columns=["ticker", "earnings_date"])
    if not merged.empty:
    ...
    gold_phases = {p.strip() for p in args.gold_phases.split(",") if p.strip()}
    ...
    for spec in specs:
        result = run_dataset(
            ...
            args.gold_ticker,
            gold_phases,
            args.gold_allocation_ratio,
            args.gold_signal_name,
            args.gold_require_recent_phase,
            args.gold_require_recent_phase_days,
            args.phase_shift_days,
            args.gold_phase_shift_days,
        )
        out_dir = args.output_dir / spec.name if args.batch else args.output_dir
        out_dir.mkdir(parents=True, exist_ok=True)
        payload = {
            "mapping_name": f"practical_phase_adaptive_v31_pre{args.earnings_pre_days}_post{args.earnings_post_days}_gold_switch",
            "mapping_ja": f"実務向け局面切替v3.1_決算前{args.earnings_pre_days}営業日_決算後{args.earnings_post_days}営業日見送り_金ETF切替",
            "earnings_pre_days": args.earnings_pre_days,
            "earnings_post_days": args.earnings_post_days,
            "gold_ticker": args.gold_ticker,
            "gold_phases": sorted(gold_phases),
            "gold_allocation_ratio": args.gold_allocation_ratio,
            "gold_signal_name": args.gold_signal_name,
            "gold_require_recent_phase": args.gold_require_recent_phase,
            "gold_require_recent_phase_days": args.gold_require_recent_phase_days,
            "phase_shift_days": args.phase_shift_days,
            "gold_phase_shift_days": args.gold_phase_shift_days,
