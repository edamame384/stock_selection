...
def run_dataset(spec: TableSpec, phase_map: pd.Series, earnings_map: dict[str, list[pd.Timestamp]], earnings_pre_days: int, earnings_post_days: int) -> dict:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run practical phase-adaptive v3.1 with pre-earnings exit and post-earnings entry cooldown.")
    ...
    rows = []
    for spec in specs:
        result = run_dataset(spec, phase_map, earnings_map, args.earnings_pre_days, args.earnings_post_days)
        out_dir = args.output_dir / spec.name if args.batch else args.output_dir
        out_dir.mkdir(parents=True, exist_ok=True)
        payload = {
            "mapping_name": f"practical_phase_adaptive_v31_pre{args.earnings_pre_days}_post{args.earnings_post_days}",
            "mapping_ja": f"実務向け局面切替v3.1_決算前{args.earnings_pre_days}営業日_決算後{args.earnings_post_days}営業日見送り",
