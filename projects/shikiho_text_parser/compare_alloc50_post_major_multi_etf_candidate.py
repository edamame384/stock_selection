...
OUT_DIR = ROOT / "projects" / "shikiho_text_parser" / "output" / "compare_alloc50_post_major_multi_etf_candidate"
...
def classify_sector_state(table_df: pd.DataFrame) -> dict:
    ...
    return {"mode": "concentrated" if concentrated else "dispersed", "top_share": top_share, "hhi": hhi, "num_sectors": num_sectors}
...
def run_dataset(spec: TableSpec, phase_df: pd.DataFrame, earnings_map: dict[str, list[pd.Timestamp]]) -> dict:
    ...
    for date in dates:
        ...
        state_rows.append({
            ...
            "score_1306": etf_scores.get("1306.T"),
            "score_1321": etf_scores.get("1321.T"),
            "score_2516": etf_scores.get("2516.T"),
        })
...
def main() -> int:
    ...
    for name, spec in specs.items():
        ...
        out_dir.mkdir(parents=True, exist_ok=True)
        payload = {
            "variant": "alloc50_post_major_multi_etf_or_stock_prev_high_break",
            "dataset": name,
            "concentrated_mode": "best of 1306/1321/2516 by weekly strength + signal gap",
