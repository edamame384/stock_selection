...
def phase_alloc_shares(fill_price: float, cash: float, phase_name: str, is_weak_uptrend: bool) -> int:
    lot_cost = fill_price * 100.0
    if lot_cost <= 0:
        return 0
    cap = PHASE_ALLOC_CAPS.get("weak_uptrend") if is_weak_uptrend else PHASE_ALLOC_CAPS.get(phase_name)
    if cap is None:
...
def run_dataset(
    ...
    for date in dates:
        ...
        for ticker, _score in candidates:
            ...
            shares = phase_alloc_shares(fill_price, cash, phase_name, is_weak_uptrend)
