...
RECOVERY_PHASES = {"settling", "reversal_up", "capitulation_end"}
HIGH_VOL_PHASES = {"high_vol", "crash"}
CORE_CONDITION2_PHASES = {"normal", "uptrend"}
CORE_BREAKOUT_PHASES = {"stable"}
...
def build_monthly_tables(detail_df: pd.DataFrame, selected_count: int, price_map: dict[str, pd.DataFrame], trade_dates: list[pd.Timestamp]) -> dict[pd.Timestamp, dict[str, pd.DataFrame]]:
    ...
    for month_start in month_starts:
        ...
        if snapshot.empty:
            empty = snapshot
            monthly[month_start] = {"core": empty, "recovery": empty, "defensive": empty}
            continue
        monthly[month_start] = {
            "core": build_table(snapshot, "regular_origcount", selected_count),
            "recovery": build_table(snapshot, "post_high_vol_origcount", selected_count),
            "defensive": build_table(snapshot, "defensive_origcount", selected_count),
        }
    return monthly
...
def table_key_for_phase(phase_name: str) -> str:
    if phase_name in HIGH_VOL_PHASES:
        return "defensive"
    if phase_name in RECOVERY_PHASES:
        return "recovery"
    return "core"


def rule_for_phase(phase_name: str) -> str:
    if phase_name in CORE_CONDITION2_PHASES:
        return "condition2"
    if phase_name in CORE_BREAKOUT_PHASES:
        return "breakout_1p5"
    if phase_name in RECOVERY_PHASES:
...
def run_backtest(spec: TableSpec, phase_map: pd.Series, output_dir: Path) -> dict:
    ...
    for month_key, tables in monthly_tables.items():
    ...
    for date in dates:
        ...
        rule_name = rule_for_phase(phase_name)
    ...
    summary = {
        "mapping_name": "operational_regime_v1",
        "mapping_ja": "運用実務レジームv1",
        "dataset": spec.name,
        "core_count": selected_count,
        "recovery_count": selected_count,
        "defensive_count": selected_count,
        ...
        "table_assignment": {
            "defensive": ["high_vol", "crash"],
            "recovery": ["settling", "reversal_up", "capitulation_end"],
            "core": "others",
        },
        "rule_assignment": {
            "condition2": ["normal", "uptrend"],
            "breakout_1p5": ["stable"],
            "q3_post_high_vol": ["settling", "reversal_up", "capitulation_end"],
            "no_trade": ["high_vol", "crash", "surge", "downtrend", "reversal_down"],
        },
    }
...
def main() -> int:
    ...
    out_root = ROOT / "projects" / "shikiho_text_parser" / "output" / "operational_regime_v1"
    specs = [
        TableSpec(
            "q2_2024",
