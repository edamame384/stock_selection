...
OUT_DIR = ROOT / "projects" / "shikiho_text_parser" / "output" / "phase_adaptive_practical_v36_2021_2023_batch"
...
def build_specs() -> list[TableSpec]:
    return [
        TableSpec(
            "q4",
            ROOT / "projects" / "quarterly_ranker" / "output" / "2021_4q_pre_analysis_20210930_candidate" / "pre_shikiho_feature_ranking.csv",
            ROOT / "projects" / "quarterly_ranker" / "output" / "2021_4q_pre_analysis_20210930_candidate" / "operational" / "pre_selected_candidates_operational.csv",
            "2021-10-01",
            "2021-12-31",
            3_000_000.0,
        ),
        TableSpec(
            "q3",
            ROOT / "projects" / "quarterly_ranker" / "output" / "2023_1q_pre_analysis_20221231_candidate" / "pre_shikiho_feature_ranking.csv",
            ROOT / "projects" / "quarterly_ranker" / "output" / "2023_1q_pre_analysis_20221231_candidate" / "operational" / "pre_selected_candidates_operational.csv",
            "2023-01-01",
            "2023-03-31",
            3_000_000.0,
        ),
...
def main() -> int:
    ...
    parser.add_argument("--dataset", choices=["2021_4q", "2023_1q", "all"], default="all")
    args = parser.parse_args()

    specs = build_specs()
    labels = ["2021_4q", "2023_1q"]
    if args.dataset != "all":
        idx = labels.index(args.dataset)
        specs = [specs[idx]]
        labels = [labels[idx]]

    phase_src = pd.read_csv(base_mod.PHASE_CSV, skiprows=[1]).rename(columns={"phase": "phase_name"})
