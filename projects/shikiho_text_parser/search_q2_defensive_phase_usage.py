...
OUT_DIR = ROOT / "projects" / "shikiho_text_parser" / "output" / "q2_defensive_phase_usage"
...
PHASE_SETS = {
    "none": set(),
    "high_vol_only": {"high_vol"},
    "crash_only": {"crash"},
    "high_vol_crash": {"high_vol", "crash"},
    "recovery_only": {"settling", "reversal_up", "capitulation_end"},
    "post_crash_full": {"high_vol", "crash", "settling", "reversal_up", "capitulation_end"},
    "plus_stable": {"high_vol", "crash", "settling", "reversal_up", "capitulation_end", "stable"},
}
...
def run_dataset(spec: TableSpec, phase_map: pd.Series, replace_phases: set[str]) -> dict:
    detail_df = normalize_static(pd.read_csv(spec.detail_csv)).drop_duplicates(subset=["ticker"], keep="first")
    standard_df = normalize_static(pd.read_csv(spec.selected_csv)).drop_duplicates(subset=["ticker"], keep="first")
    standard_tickers = set(standard_df["ticker"].astype(str))
    ...
    for month_key in month_starts:
        ...
        q2_tables[month_key] = build_q2_defensive_table(build_snapshot(detail_df, price_map_all, basis_date))

    union_tickers = set(standard_table["ticker"].astype(str))
    for tbl in q2_tables.values():
        if not tbl.empty:
    ...
    for date in dates:
        ...
        use_q2_def = phase_name in replace_phases
        active_lookup = q2_tables[month_key].set_index("ticker").to_dict("index") if use_q2_def and not q2_tables[month_key].empty else standard_lookup
        rule_name = "q2_defensive" if use_q2_def else mapping.get(phase_name, "condition2")
...
def main() -> int:
    ...
    for name, phases in PHASE_SETS.items():
        row = {"phase_set_name": name, "replace_phases": ",".join(sorted(phases))}
        total_final = 0.0
        for spec in specs:
            result = run_dataset(spec, phase_map, phases)
        ...
        row["total_final_capital"] = total_final
        row["total_return_pct"] = (total_final / (3_000_000.0 * len(specs)) - 1.0) * 100.0
        rows.append(row)

    df = pd.DataFrame(rows).sort_values("total_final_capital", ascending=False)
    df.to_csv(OUT_DIR / "search_results.csv", index=False, encoding="utf-8-sig")
