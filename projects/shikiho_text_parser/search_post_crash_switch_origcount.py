...
OUT_DIR = ROOT / "projects" / "shikiho_text_parser" / "output" / "post_crash_switch_origcount"
...
def build_table(df: pd.DataFrame, table_name: str, top_n: int) -> pd.DataFrame:
    x = df.copy()
    ...
    if table_name == "regular_origcount":
    ...
    if table_name == "momentum_origcount":
    ...
    if table_name == "post_high_vol_origcount":
    ...
    if table_name == "defensive_origcount":
        x = x[x["trend_r2"].fillna(0) >= 0.35]
        x = x[x["annual_return_pct"].fillna(-999) >= 0.0]
        x = x[x["quarter_return_pct"].fillna(-999) >= -10.0]
        x = x[x["positive_month_ratio_pct"].fillna(0) >= 35.0]
...
def run_dataset(spec: DatasetSpec, phase_map: pd.Series, post_table_name: str, other_table_name: str, post_rule_name: str, other_rule_name: str) -> dict:
    ...
    original_df = pd.read_csv(spec.original_selected_csv)
    original_count = int(original_df["ticker"].nunique()) if "ticker" in original_df.columns else int(len(original_df))
    tables = {
        "post": build_table(universe, post_table_name, original_count),
        "other": build_table(universe, other_table_name, original_count),
    }
        ...
        for ticker, sig in signal_today.items():
            ...
            if sig and not prev_signal.get(ticker, False):
                ...
                if high >= entry_trigger:
                    ...
                    candidates.append((ticker, score_today.get(ticker, 0.0), entry_px, regime, post_rule_name if regime == "post" else other_rule_name))

        candidates.sort(key=lambda x: x[1], reverse=True)
        for ticker, _score, entry_px, _regime, rule_name in candidates:
            shares = int(cash // (entry_px * 100.0)) * 100
            if shares < 100:
                continue
            cash -= shares * entry_px
            positions[ticker] = {"shares": shares, "entry_price": entry_px, "rule_name": rule_name}
            buy_count += 1
...
def main() -> int:
    ...
    post_tables = ["none", "post_high_vol_origcount", "defensive_origcount"]
    other_tables = ["regular_origcount", "momentum_origcount"]
    ...
    for post_table_name, other_table_name, post_rule_name, other_rule_name in itertools.product(post_tables, other_tables, post_rules, other_rules):
        ...
        for r in results:
            ...
            row[f"{r['dataset']}_post_count"] = r["post_count"]
            row[f"{r['dataset']}_other_count"] = r["other_count"]
            row[f"{r['dataset']}_original_count"] = r["original_count"]
        rows.append(row)
