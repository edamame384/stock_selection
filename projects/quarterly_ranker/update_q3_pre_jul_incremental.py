...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Incrementally rebuild full 3Q pre-Jul selection using an existing library as a base."
    )
    ...
    if seed_library.empty:
        seed_library = pd.DataFrame(columns=["image_file", "image_no", "ocr_metrics_text", "code", "ticker", "company_name", "sector", "resolved"])
        max_existing_image_no = 0
    else:
    ...
    if batch_images:
        new_library = build_3q_library(incremental_dir, name_master, args.out_dir / "library_cache")
    ...
    base_df = build_pre_jul_base(full_library, args.price_dir)
    ...
    summary_csv = args.out_dir / "q3_learning_dataset_manifest.csv"
