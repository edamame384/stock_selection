...
def build_3q_library(image_dir: Path, name_master: pd.DataFrame, cache_dir: Path) -> pd.DataFrame:
...
def calc_pre_jul_metrics(symbol: str, price_path: Path) -> dict[str, float] | None:
    ...
    q_start = pd.Timestamp("2025-04-01")
...
def build_pre_jul_base(library: pd.DataFrame, price_dir: Path) -> pd.DataFrame:
    rows = []
    for _, row in library[library["resolved"] == True].iterrows():
        ...
        if not price_path.exists():
            continue
        metrics = calc_pre_jul_metrics(ticker, price_path)
        if metrics is None:
            continue
    ...
    for _, row in df.iterrows():
        ...
        if price_path.exists():
            ...
            if "Date" in px.columns and "Close" in px.columns:
                ...
                test = px[(px["Date"] >= "2025-07-01") & (px["Date"] <= "2025-09-30")].copy()
                if not test.empty:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Select promising 3Q stocks using only data up to 2025-06-30.")
    ...
    parser.add_argument("--out-dir", type=Path, default=Path("projects/quarterly_ranker/output/q3_pre_analysis"))
    parser.add_argument("--reuse-library", action="store_true")
    args = parser.parse_args()

    args.out_dir.mkdir(parents=True, exist_ok=True)
    name_master = load_name_master(args.name_csv)
    library_csv = args.out_dir / "q3_image_library.csv"
    if args.reuse_library and library_csv.exists():
        library = pd.read_csv(library_csv)
    else:
        library = build_3q_library(args.image_dir, name_master, args.out_dir / "library_cache")
        library.to_csv(library_csv, index=False, encoding="utf-8-sig")

    base_df = build_pre_jul_base(library, args.price_dir)
    ...
    detail.to_csv(args.out_dir / "q3_pre_shikiho_feature_ranking.csv", index=False, encoding="utf-8-sig")
    selected = select_candidates(detail)
    selected = attach_realized_returns(selected, args.price_dir)
    selected.to_csv(args.out_dir / "q3_pre_selected_candidates.csv", index=False, encoding="utf-8-sig")
    ...
    print(f"[OUT] detail={args.out_dir / 'q3_pre_shikiho_feature_ranking.csv'}")
    print(f"[OUT] selected={args.out_dir / 'q3_pre_selected_candidates.csv'}")
