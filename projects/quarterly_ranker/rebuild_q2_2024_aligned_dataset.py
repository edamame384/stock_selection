...
def calc_q2_2024_metrics_aligned(symbol: str, price_path: Path) -> dict[str, float] | None:
...
def build_q2_2024_base_aligned(library: pd.DataFrame, price_dir: Path) -> pd.DataFrame:
    rows = []
    for _, row in library[library["resolved"] == True].iterrows():
        ...
        if not price_path.exists():
            continue
        metrics = calc_q2_2024_metrics_aligned(ticker, price_path)
        if metrics is None:
            continue
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Rebuild 2024-2Q candidates with aligned 252/63-day price windows up to 2024-06-30.")
    parser.add_argument("--library-csv", type=Path, default=Path("projects/quarterly_ranker/output/q2_2024_pre_analysis_20240630_full/q2_2024_image_library.csv"))
    ...
    parser.add_argument("--out-dir", type=Path, default=Path("projects/quarterly_ranker/output/q2_2024_pre_analysis_20240630_aligned"))
    parser.add_argument("--learning-date", default="2024-06-30")
    args = parser.parse_args()

    args.out_dir.mkdir(parents=True, exist_ok=True)
    library = pd.read_csv(args.library_csv)
    library.to_csv(args.out_dir / "q2_2024_image_library.csv", index=False, encoding="utf-8-sig")

    base_df = build_q2_2024_base_aligned(library, args.price_dir)
    ...
    summary = pd.DataFrame([
        {
            "dataset_id": "q2_2024_pre_analysis_20240630_aligned",
        ...
        },
        {
            ...
            "image_count_total": int(len(library)),
            "selected_count": None,
            "output_dir": "projects/quarterly_ranker/output/q2_2024_pre_analysis_20240630_full",
            "source_image_dir": str(args.image_dir),
            "source_library_csv": str(args.library_csv),
        },
    ])
    summary.to_csv(args.out_dir / "q2_2024_learning_dataset_manifest.csv", index=False, encoding="utf-8-sig")
