...
from projects.quarterly_ranker.select_2024_q2_pre_jul_candidates import (
    ...
    build_2024_q2_library,
    build_pre_jul_2024_base,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Incrementally rebuild 2024-2Q selection using 2024-06-30 cutoff.")
    ...
    if library_csv.exists():
        ...
        existing_library = pd.DataFrame(columns=["image_file", "image_no", "ocr_metrics_text", "code", "ticker", "company_name", "sector", "resolved"])

    name_master = load_name_master(args.name_csv)
    processed_image_nos = set(existing_library["image_no"].astype(int)) if not existing_library.empty else set()
    pending_images = []
    ...
    if batch_images:
        new_library = build_2024_q2_library(incremental_dir, name_master, args.out_dir / "library_cache")
    ...
    if remaining_after_batch > 0:
        print(f"[INFO] library_progress processed_total={len(full_library)} processed_this_run={len(batch_images)} remaining={remaining_after_batch}")
        print(f"[OUT] library={library_csv}")
        return 0

    base_df = build_pre_jul_2024_base(full_library, args.price_dir)
    ...
    summary_csv = args.out_dir / "q2_2024_learning_dataset_manifest.csv"
    ...
    print(f"[INFO] images={len(full_library)} selected={len(selected)}")
    return 0
